...
AVAILABLE_TOOLS = [
    {
        "name": "remember_note",
        "description": "Write an important note into long-term memory (RAG). Use it to remember facts, strategies, events, or tips that may be useful later. Do not duplicate notes that are already present.",
        "input_schema": {
            "type": "object",
            "properties": {
                "text": {
                    "type": "string",
                    "description": "The note to save (max 300 characters)."
                },
                "tags": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "(Optional) Tags or categories associated with the note."
                }
            },
            "required": ["text"],
        },
...
AVAILABLE_TOOLS.append({
    "name": "delete_remember_note",
    "description": "Delete one or more notes saved in long-term memory (RAG). Provide the 'timestamp' field to remove a specific note, or the 'text' field to remove all notes that exactly match the given text. Use this only when you are sure the saved note is incorrect or obsolete.",
    "input_schema": {
        "type": "object",
        "properties": {
            "timestamp": {
                "type": "number",
                "description": "(Optional) Exact timestamp (epoch) of the note to delete."
            },
            "text": {
                "type": "string",
                "description": "(Optional) Exact text of the note to delete."
            },
            "confirm": {
                "type": "boolean",
                "description": "(Required) Explicit confirmation to allow deletion. Must be true to perform the deletion."
            }
        },
        "required": []
    },
...
AVAILABLE_TOOLS.append({
        "name": "opinion",
        "description": "Send an opinion to the other agent. This is shared in the next payload.",
        "input_schema": {
            "type": "object",
            "properties": {
                "to": {
                    "type": "string",
                    "enum": ["explorer", "trainer"],
                    "description": "Which agent should receive this opinion"
                },
                "opinion": {
                    "type": "string",
                    "description": "Your opinion, advice, or concern"
                }
            },
            "required": ["to", "opinion"],
        },
...
AVAILABLE_TOOLS.append({
        "name": "objective",
        "description": "Set or update the shared objective. This is included in both agents' payloads.",
        "input_schema": {
            "type": "object",
            "properties": {
                "objective": {
                    "type": "string",
                    "description": "The shared objective or next goal"
                }
            },
            "required": ["objective"],
        },
